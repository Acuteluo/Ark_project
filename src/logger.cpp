...
void set_logger()
{
    ...
    // 2. 配置文件输出 Sink
    auto file_name = fmt::format("{}/Ark_project_{:%Y-%m-%d_%H-%M-%S}.log", log_dir, std::chrono::system_clock::now());
    ...
    // 4. 组合 Sinks 并注册
    logger_ = std::make_shared<spdlog::logger>("Ark_Logger", spdlog::sinks_init_list{file_sink, console_sink});
